...
class AuraxisEngine:
    ...
    def get_price_redundancy(self):
        try:
            res = requests.get("https://api.frankfurter.app/latest?from=EUR&to=USD", timeout=3).json()
    ...
    def get_data_v10(self, ticker="EURUSD=X"):
        try:
            ...
            if self.preco_suavizado is None: self.preco_suavizado = p_bruto
            else: self.preco_suavizado = (p_bruto * self.alfa) + (self.preco_suavizado * (1 - self.alfa))
        ...
        except: return pd.DataFrame(), 0.0, 0.0, "ERRO"
    ...
    def calculate_radar(self, df, mode="DAY", trend_direction=0, p_refinado=None):
        ...
        if mode != "POSITION" and trend_direction != 0:
            if (trend_direction > 0 and z_score < 0) or (trend_direction < 0 and z_score > 0): return None
        atr = (df['high'] - df['low']).rolling(p).mean().iloc[-1]
        z_inf, z_sup = p_calc - (atr * 0.4), p_calc + (atr * 0.4)
        if z_score > 1.3:
            return {"tipo": "COMPRA", "z_inf": z_inf, "z_sup": z_sup, "tp": [p_calc + (atr * m), p_calc + (atr * m * 1.3)], "sl": [p_calc - (atr * m * 0.7), p_calc - (atr * m)], "prob": min(65 + (z_score * 4), 98.8), "z": z_score}
        elif z_score < -1.3:
            return {"tipo": "VENDA", "z_inf": z_inf, "z_sup": z_sup, "tp": [p_calc - (atr * m), p_calc - (atr * m * 1.3)], "sl": [p_calc + (atr * m * 0.7), p_calc + (atr * m)], "prob": min(65 + (abs(z_score) * 4), 98.8), "z": z_score}
        return None
